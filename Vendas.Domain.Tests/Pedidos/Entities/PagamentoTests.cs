...
namespace Vendas.Domain.Tests.Pedidos.Entities
{
    public class PagamentoTests
    {
        ...
        public void Deve_Criar_Pagamento_Valido_Com_Status_Pendente()
        {
            ...
            var valor = new ValorMonetario(100m);
        ...
        public void Nao_Deve_Criar_Pagamento_Com_Valor_Invalido()
        {
            ...
            //Action act = () => new Pagamento(pedidoId, MetodoPagamento.Pix, valor);

            Action act = () => new ValorMonetario(0);

            act.Should().Throw<DomainException>().WithMessage("O Valor do pagamento deve ser maior que zero.");
        }

        [Fact(DisplayName = "Não deve definir código transação nulo ou vazio")]
        public void Nao_Deve_Definir_Codigo_Transacao_Nulo_Ou_Vazio()
        {
            var pagamento = new Pagamento(Guid.NewGuid(), MetodoPagamento.Pix, new ValorMonetario(100m));

            Action act = () => pagamento.DefinirCodigoTransacao("");

            act.Should().Throw<DomainException>().WithMessage("Código da transação inválido.");
        }

        [Fact(DisplayName = "deve definir código transação válido e atualizar DataAtualizacao")]
        public void Deve_Definir_Codigo_Transacao_Valido()
        {
            var pagamento = new Pagamento(Guid.NewGuid(), MetodoPagamento.CartaoCredito, new ValorMonetario(100m));
        ...
        [Fact(DisplayName = "Não deve redefinir código transação já definido")]
        public void Nao_Deve_Redefinir_Codigo_Transacao()
        {
            ...
            act.Should().Throw<DomainException>().WithMessage("O código da transação já foi definido.");
        }

        [Fact(DisplayName = "Deve gerar código transação local automaticamente")]
        public void Deve_Gerar_Codigo_Transacao_Local()
        {
            var pagamento = new Pagamento(Guid.NewGuid(), MetodoPagamento.CartaoCredito, new ValorMonetario(200m));
        ...
        {
            var pagamento = new Pagamento(Guid.NewGuid(), MetodoPagamento.CartaoCredito, new ValorMonetario(300m));
        ...
        [Fact(DisplayName = "Não deve confirmar pagamento que não está com status pendente")]
        ...
        [Fact(DisplayName = "Deve recusar pagamento pendente e gerar evento com dados")]
        ...
        [Fact(DisplayName = "Deve recusar pagamento que não está pendente")]
        public void Deve_Recusar_Pagamento_Que_Nao_Esta_Pendente()
        {
            var pagamento = new Pagamento(Guid.NewGuid(), MetodoPagamento.CartaoCredito, new ValorMonetario(120m));
